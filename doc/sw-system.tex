...
companion paper (More et al, in preparation, herafter \papertwo) we will
...
have the same properties of the training set. In \papertwo we carry
...
In \Fref{fig:training-gallery} we show 12 example training images from the
first \sw project (\Sref{sec:data} below). 

\todo{Phil, Aprajita}{Make gallery of 6 stage 2 sims and 6 stage 2 duds, to
give the reader a feel for what the sims look like, and also how the images
were presented, stretch and scale-wise.}
...
\todo{Phil}{write this section (\#31)}
...
images can be seen in \Fref{fig:training-gallery}.
...
to quantify the efficiency of the system.  The contribution per classification
is defined in terms of a hypothetical subject with lens probability of 0.5;
one bit of information is needed to update such a subject's lens probability
to either zero or one. This means that a maximally complete classification
stage would yield a total contribution (summed over all agents) equal to the
number of subjects. The ratio of this hypothetical optimum to the actual total
...
remained in the system, being over-classified. Indeed, only 3705745
classifications were needed to retire all the subjects: the ratio of this to
the total number made is 34\%. (The remaining 1\% is due to not all subjects
being classified to 1 or 0 probability.) At stage 2, we did not retire any
subjects at all; the inefficiency in this case was by design, to give everyone
a chance to appreciate what they had found together! 
...
\todo{Chris}{complete the above sentence to explain what purity means in the
divided-by-sim-type case. Thanks! \#66}
...
\todo{Chris}{fill in the numbers above, please! :-) \#65}
...
We first investigate the source of the incompleteness visible in 
\Fref{fig:results:sample:CP}, by examining the stage 2 false negatives.
\Fref{fig:discuss:performance:FNgallery} shows N example simulated lenses that
were missed by the \sw system. In some cases their rejection is
understandable: the lensed features are faint, or otherwise difficult to see.
However, in other cases a reasonably obvious lens was passed over. 
\Fref{fig:discuss:performance:FNtrajectories} shows the probability 
trajectories of these subjects. \todo{Phil}{Make the FN plots, and look for
behaviour. Can we see whether the FNs are due to costly mistakes by expert
classifiers? Or are they just random walking south?}

Is the system's impurity a result of the same behaviour?
Figures~\ref{fig:discuss:performance:FPgallery}
and~\ref{fig:discuss:performance:FPtrajectories} show the corresponding
figures for the false positives -- non-lenses that were incorrectly inferred
to be lens candidates during stage 2. \todo{Phil}{Make the FP plots, and look
for behaviour. Can we see whether the FPs are due to costly mistakes by expert
classifiers? Or are they just random walking south?}

In general, there are three places we can act to improve system performance:
educating the volunteers, assigning subjects for classification, and
interpreting the classification data. The plots in this section suggest that
...
\todo{Phil}{complete this thought, and update the text below, in the light of
the FP and the FN figures} 

Mistakes by reasonably high skill classifiers cause large jumps in subject
probability in the wrong direction, as are occasionally visible in the
trajectory plots in this section. We might wonder whether some of those
mistakes could have been corrected by the classifiers themselves, had they had
access to a ``go back'' option. While clearly enabling error reduction, we
might worry about such a mechanism enabling classifiers to have a guess, then
go and read the Talk discussion, and then return to the classification
interface to change their mind to agree with the forum. In fact, this is
already possible on the current interface -- and indeed, it is not obvious
that such correlations would necessarily lead to poorer system performance.
This could be tested by presenting a fraction of the volunteers with a version
of the site that actively suggested that they take this approach, and then
tracking the relative performance of ``collaborative'' classifications
compared with independent ones. We leave this to further work.  
...
to see more training images (still at random) in their stream. Another could
be to provide more information about what lenses do. In this project, the
Spotter's Guide was always available on the site, but as a passive background
resource. We might consider providing more links to this guide in the feedback
messages shown to the classifiers as they go -- or perhaps extending these
messages to themselves include more explanations and example images. 
...
Some trajectories show signs of a random walk problem. Those subjects are
simply ``unlucky:'' they are only seen by low skill classifiers, and  happen
to be mis-classified  enough times to cross the retirement threshold (in the
case of the false negatives) or remain in the system (in the case of the false
positives). While improved training will help reduce this effect, it is here
that targeted task assignment could help improve system performance. We
...
skill (since stage 2 was quite short): of
the 1964 volunteers who took part in the stage 2 classification round, only
774 were veterans from stage 1. \Fref{fig:discuss:performance:stage1vstage2}
shows how the stage 1 skill maps on to stage 2 skill and contribution...

\todo{Phil}{Remake figure to show stage 2 skill vs stage 1 skill with point
size given by stage 2 contribution. Stage 2 newbies will need to be plotted to
have (0 +/- small) stage 1 skill, to show up as a cloud.}

Thsi figure suggests that there may be some benefit to opening stage 2 on the
basis of agent skill, in order to reduce the noise in the system generated by
new and low skill volunteers in this more difficult classification stage. A
simple implementation of this would be to switch the web app to the stage 2
version when either a certain stage 1 skill (and/or effort) threshold was
passed. Good strategy would probably be to switch back to stage 1 after some
number of stage 2 classifications (to avoid losing the high skill users from
stage 1 entirely), and also to make it very clear which stage the classifier
was at (since stage 2 requires more careful attention). This is something we
plan to experiment with in future.

Finally, there could be some performance gains to be made by improving the
SWAP agent model, or its implementation. In a future paper we plan to
investigate the use of the test images as well as the training images in
learning the agent confusion matrix elements (Davis et al, in preparation).
One might also consider looking at introducing more conditional dependences in
these elements, to allow for some classifiers having greater skill in one type
of lens than another, or, more generally, as a function of lens property (such
as color, brightness, and image separation). In the current model, all agents
are considered to be completely independent, whereas in fact we might expect
there to be significant clustering of the confusion matrix elements. A
hierarchical model for the crowd, with hyper-parameters describing the
distribution of confusion matrix elements across the population, may well
accelerate the agent learning process by including the notion of one agent
being likely to be similar to its neighbours. Finally, it is worth noting that
the model of \citet{IBCC} explicitly avoids the assumption of the agent
confusion matrix elements being constant in time, allowing the development of
volunteer skill to be more accurately tracked -- and that they did see some
time-evolution in the supernova zoo classifiers' skill. Finding a way to
incorporate such a learning model into SWAP, while retaining its statistically
online character is an interesting challenge for future work.
...
Inefficiency. Move to truly online system: web app to analyse classifications
as it goes. Place for weekly offline processing? Recovering false negatives by
``resurrecting'' subjects that scattered down too soon.
...
Scaling of system. Factors of ten: $10^4$ volunteers, $10^3$ achieve high
skill, $10^2$ make significant contribution. Good project size is few $10^4$
subjects, to enable completion by a crowd of this size in short time. 

Ways to increase numbers of high-contribution volunteers: 1) increase crowd
size. Higher exposure through media. Multi-lingual user base - comes with
problems.  2) Increase engagement rate:  feedback from survey teams. 
