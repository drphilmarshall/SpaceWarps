...
were known to be unrealistic. The fraction of such unrealistic simulations in the 
...
At group-scales, the photometric and redshift estimates are used when defining
...
which have features similar to lenses but they are not lenses in reality, for example, spiral galaxies,
...
and a sample of 500 false positives for the \StageTwo\footnote{Please see \Sref{sec:swap} and Paper 1 for a description of \StageOne and \StageTwo} inspection. The
...
duplication, only the newly discovered lens
candidates with G$\ge 1.3$ (medium-high probability) are presented in
this paper (see \Sref{sec:results:newcand}), and further information on \sw-selected candidates that were previously
identified in the literature (see \Sref{sec:data:kls}) will be made available at
...
candidates (ranked with medium-high probability) found by \sw in
...
allowed for an arc to be typically detected. Human brains have no
...
\sw {\bf interface did not show} any galaxy subtracted images and hence,
...
are missed both at \StageOne and II (see \Tref{tab:stats}). In fact, all
...
The {\bf top-left panels of \Fref{fig:detmis}} show the detected lenses. The cases shown with green trajectories 
