...
\new{Because we include a training set of images in amongst the survey ``test''
images in this way, \sw can be viewed as a {\it supervised learning} system.
...
\new{Second, the lensed features themselves must be realistic; if they did not
...
\new{We start by selecting massive galaxies and groups in the CFHTLS catalogs,
by color, brightness and photometric redshift. We then assign plausible mass
...
\new{At this point, the distribution of lens and source properties are quite
realistic. At this point, we chose to make several cuts, to reject sims that
were likely to be difficult to spot. While this ensured the educational value
of the sims when giving feedback, it meant that we should only {\it expect} to
detect real lens candidates that also pass these cuts. Any real lenses
candidates we find with properties outside the ranges of the sims may in some
sense be considered bonuses, a product of the {\it imagination} of the crowd
in extrapolating their understanding of gravitational lenses into a regime
...
that $i = 23$ and b) magnification greater than 20 (\PaperTwo, Table 1).}
...
images were also available for perusal in Talk). \new{We made colour composite PNG
format images
following the prescription of \citet{LuptonEtal2004} \citep[with extensions
by][and some particular choices of our own]{WherryEtal2004}, using the \humvi
software.\footnote{The \humvi colour image composition code used in
this work is open source and available from \texttt{http://github.com/drphilmarshall/HumVI}} Specifically, we
first rescaled the pixel values of each image, in the notation of
\citeauthor{LuptonEtal2004}) into flux units (``picomaggies,''), via the image
AB zeropoint $m_0$ and the pixel value calibration factor $f$  given by
$\log_{10}{f} = 0.4(30.0 - m_0)$. We then applied further aesthetic
``scales'' $s_{i,r,g}$ before computing the intensity image $I$ and applying
an arcsinh stretch. (The scales are re-normalized to sum to one.) Thus, the red
($R$), green ($G$) and blue ($B$) channel images correspond to the CFHTLS $i$,
$r$ and $g$-band images in the following way:
\bea
...
recommendation of \citet{LuptonEtal2004}, but we found it to give very
informative but also familiar-looking astronomical images.}

\new{The non-linearity parameters $Q$ and $\alpha$ control the brightness and
...
in terms of the information  contributed by the crowd in
\Sref{sec:results:crowd}, the speed of the system, and the overall
classifications of the training set that they made.
...
agents, and plot their distributions in
