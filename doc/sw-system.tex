...
imaging survey into some 430,000 overlapping 84 by 84 arcsecond tiles and
...
\citep[\cfhtls,][]{Gwyn2012} ``Wide'' fields cover a total of
171 square degrees of sky.
...
(anonymous) volunteer IDs (their Zooniverse usernames), a timestamp and the
classification results (the image pixel-coordinate positions of every marker
placed).  The ``category'' of subject -- whether it is a ``training'' subject (a
simulated lens or a known non-lens) or a ``test'' subject (an unseen image drawn
from the survey) -- is also recorded. For training subjects, we also store the
...
classification of every subject based on all image views to date.  Not all
...
detection threshold were also subsequently ignore, but they were not
...
If the system efficiency could be improved by a factor of three, we would
expect the coefficient in \Eref{eq:speed} to decrease by roughly the same factor,
suggesting that an dataset of $10^4$~images could be searched for lenses by a
crowd of $10^4$ volunteers in about 6 days.
