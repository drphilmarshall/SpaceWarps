...
training set is used to give the volunteers instantaneous feedback on their performance, as
well as to calibrate a model of the system that provides
dynamical updates to the probability that a classified image contains a lens. Low probability systems are
retired from the site periodically, concentrating the sample towards a set of lens
...
months. This \StageOne search reduced the sample to 3368 images containing candidates; these were then
refined in \StageTwo to yield a sample that we expect to be over 90\% complete and 30\% pure, based on our analysis of the volunteers performance on training images (refer for \Sref{sec:design:stages} for more information on the two stage strategy) .
We comment on the scalability of the \SW system to the wide field survey era,
based on our finding that searches of $10^4$ images can be performed by a
crowd of $10^4$ volunteers in 6 days.
\end{abstract}
...
Until this problem is solved by machine learning tools, at present visual image inspection seems unavoidable at some level when
...
\flag{AV}{Do we say somewhere that once a subject reaches 0.95 it cannot be down
graded even if there are more classifications?}
...
\flag{AV}{MENTION HARDER TRAINING HERE MEANING THAT THE SPREAD IS LARGER?}
...
decrease
in participation later on.) \StageTwo (3400 subjects) was completed in around 600
...
\flag{AV}{wonder if we should include some definitions for less familiar readers
for false positive: a non-lens that is selected by the crowd,
false negative: a lens candidate that is rejected by the crowd,
true positive: a lens candidate selected by the crowd,
true negative (not used in the paper): an empty field that's correctly
identified as being empty...}
...
they had access to a ``go back'' option. While clearly enabling error reduction, particularly those caused by simply skimming too fast,
...
correlations would necessarily lead to poorer system performance.
\flag{AV}{REPETITION OF
TALK FACTOR FROM EARLIER SECTION, IT"S NOT NEEDED TWICE}. There may well
also be effects on volunteer confidence; it may be that encouraging this sort of
...
However, identification failures \flag{AV}{DEFINE WHAT YOU MEAN BY IDENTIFICATION
FAILURES} around 60\%, from inspection of a random sample) seem to be due to the
statistical noise associated with the many short, semi-random direction kicks
arising from classifications made by low skill, inexperienced classifiers
(\Sref{sec:results:crowd}). \flag{AV}{ARE YOU SAYING THAT THERE ARE MORE OF THESE THAN THE
RIGHT COLUMN EXAMPLES?  WE SHOULD GIVE NUMBERS.} While improved training will
help reduce this effect, it is here that targeted task assignment could also
help improve system performance, by bringing higher skill volunteers in when
needed. We advertised \StageTwo of the current project to all registered users;
it was taken up by \StageOne veterans with a broad range of skill, and also
picked up a significant number of new users who did not have enough time to
...
rate within reach of \RF, for example:][]{GavazziEtal2014}. Increasing $\tau_0$
...
(Stages I and II). This FPR translates to an approximate purity of 30\%. We find a
