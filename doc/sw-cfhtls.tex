...
lens galaxies are robust. \new{ \sw recovers about 65\% of known lenses, while
...
\new {Statistics of detections in \sw} }
\begin{tabular}{l l l l l}
\hline
...
\new{In \Tref{tab:stats}, we give overall statistics of the systems
...
each stage.  Overall, the sample of new \sw lens candidates comprises
over 40~per~cent of the total \sw$-$\cfhtls lens candidates.  Nearly
45~per~cent of the known lens candidates from \StageOne are rejected at
\StageTwo. Such fractions are acceptable for ``candidates'' as their
quality grades vary from high to low.  We find that most of the
confirmed lenses found at \StageOne are also recovered at \StageTwo.
However, $\sim35$~per~cent of the known lenses were missed already at
\StageOne: we return to the discussion of these false negatives below in
\Sref{sec:fn}.
}
...
choose to maximize \new{the purity of the final sample. However,
...
\new{ \Tref{tab:stats}, we show that $\sim32\%$ of the known lens candidates,
...
\new{Like any lens finding method, the \sw system could potentially be
...
\new{Many of the missed lenses are from the \rf sample with small
...
of \Fref{fig:detmis}, show visually easier (and more difficult) to identify
...
\new {\item We use a sample of simulated lenses, duds and impostors tailored
...
\new{\item Based on the known sample of lenses and lens candidates, we find
that we lose a small fraction of them during \StageTwo refinement.
It is more important to improve the lens
detection sensitivity at the initial \StageOne classification step.
About 35\% of the known lenses (20 in total) were missed at \StageOne.
Two thirds of these missed lenses were found to be galaxy-scale \rf
...
\new{Upcoming and planned wide-field imaging surveys such as the DES, HSC and
...
volunteers which can be extrapolated to the entire survey. However, any
one approach for finding lenses from the entire survey data is likely
going to be inefficient.} Thus, combining robotic methods for
pre-selection with the citizen science approach for visual screening
could be a good strategy for finding lenses in these large imaging
surveys.
